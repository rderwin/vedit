...
- `freezes: [{t, dur}]` — pause the video on a single frame while audio continues. Implemented as an auto-cutaway: the assembler extracts the frame at clip-relative `t` and uses it as a cutaway image for `dur` seconds. The classic comedy / reaction move. Pairs naturally with `stings` and `overlays` firing on the same beat.
- `punches: [{t, intensity, dur?}]` — explicit reaction-zoom snap-ins at clip-relative `t`. Sharper envelope than `auto_zoom` (snap up over 0.10s, hold, release over 0.40s) and goes harder (max 1.20× vs auto_zoom's 1.06×). `intensity` 0..1 scales how aggressive the zoom is; `dur` (default 0.5s) is the total visible time. Use when you want a punch, not a ramp — landings on jokes, reaction beats, "wait, what??" moments. Combines with `auto_zoom` via max() so concurrent peaks don't compound.
- `shake: [{t, intensity, dur?}]` — handheld camera shake on impact. Adds time-bounded sinusoidal x/y offsets (sin 40Hz / cos 53Hz so the wobble isn't perfectly circular) plus a small base zoom for crop headroom. `intensity` 0..1, `dur` default 0.4s, triangle envelope (fades in and out). Pairs naturally with `punches` and `stings` firing on the same beat — the boom hits, the camera reacts.
- `flashes: [{t, color?, intensity?, dur?}]` — full-canvas color flash overlay for an impact pop. `color` accepts ffmpeg color names (`white`, `black`, `red`) or `#RRGGBB` (default `white`); `intensity` 0..1 is the alpha (default 1.0); `dur` defaults to 50ms (~1.5 frames at 30fps). Sits below captions/title so text stays readable through the flash. Combine with `shake` + `punches` + a `boom` sting at the same `t` for the maximum-impact compound move.
- **Punchline emphasis** (automatic, no key) — in `caption_mode: word` or `word_active`, words that are ALLCAPS (≥3 letters) or end with `!!` / `??` / `?!` / `!?` render bigger and in the accent color. Hand-edit `transcript.json` to flag specific words: change `"that was unreal"` → `"that was UNREAL"` to make the caption pop on the punchline. Whisper sometimes preserves shouting as ALLCAPS automatically.
